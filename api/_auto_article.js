import { requireConfig } from "./_supabase.js";
...
const DAY_POWER_NUMBERS = { 0: 6, 1: 18, 2: 8, 3: 17, 4: 19, 5: 21, 6: 10 };

export function resolveSlot(slotParam) {
  if (
    slotParam === "morning" ||
    slotParam === "forenoon" ||
    slotParam === "noon" ||
...
function buildFallbackArticle(slot, dateStr) {
  ...
  // Salt ensures numbers generated across the 5 slots on the exact same date NEVER collide
  ...
  if (slot === "morning") {
    return {
      ...
      seoDescription: `แนวทางเลขเด็ดรอบเช้าประจำ${thaiDate} เลขมงคลเสริมโชคลาภ เลขเด่น 2 ตัว ${pair1}, ${pair2} และ 3 ตัวตรง ${triple1}`,
...
function buildGeminiPrompt(slot, dateStr) {
  const thaiDate = formatThaiDate(dateStr);
  const slotConfig = {
    morning: {
...
export async function generateLuckyNumberArticle(options = {}, restFunction) {
  const slot = resolveSlot(options.slot);
  const dateStr = options.targetDate || getBangkokDateString();
  const slug = `lek-ded-${slot}-${dateStr}`;

  if (!options.force) {
    const existing = await restFunction(`articles?slug=eq.${encodeURIComponent(slug)}&select=slug,title`);
  ...
  const saveResponse = await restFunction("articles?on_conflict=slug", {
    method: "POST",
  ...
  if (!saveResponse || !saveResponse.ok) {
    throw new Error(`บันทึกบทความไม่สำเร็จ: ${saveResponse?.error || "network error"}`);
  }
